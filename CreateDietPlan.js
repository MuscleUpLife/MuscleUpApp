      ...
      meals.forEach((meal) => {
        ...
        if (extractedData[meal]) {
          extractedData[meal].forEach((item, index) => {
            ...
            const foodLines = wrapText(item.food, 35);

            foodLines.forEach((line, lineIndex) => {
              page.drawText(sanitizeText(line), { x: 40, y: yOffset - lineIndex * 12, size: 12, font: regularFont, color: rgb(0, 0, 0) });
            });
            ...
            yOffset -= itemSpacing + (foodLines.length - 1) * 12;
